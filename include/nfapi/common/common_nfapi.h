...
enum nf_ip_addr_type {
	NF_IPA_SINGLE = 0,/*!< Single IP address */
...
struct nf_ipv4_addr_info {
	...
	union {
		...
		struct {
			...
			nf_ipv4_addr net_mask; /*!< Subnet mask */
		} subnet;
...
/*! Port type. */
enum nf_port_type {
	NF_PORT_SINGLE = 0,		/*!< Single port */
	NF_PORT_RANGE,		/*!< Range of ports */
	NF_PORT_SERVICE,		/*!< Standard or custom service port */
	NF_PORT_SERVICE_GRP		/*!< Service group */
};

/*! Data type for service single/range/object. */
struct nf_port {
	uint8_t	type;		/*!< Type of port data.
                                 * See "enum nf_port_type". */
	union {
		struct {
